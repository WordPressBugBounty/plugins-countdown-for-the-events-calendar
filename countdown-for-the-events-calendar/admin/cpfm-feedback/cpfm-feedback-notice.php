...
class CPFM_Feedback_Notice {
    ...
    public function cpfm_enqueue_assets() {
       ...
       $current_page   = isset($_GET['page'])? sanitize_key($_GET['page']):'';
        ...
        wp_enqueue_style(
            ...
            TECC_PLUGIN_URL . 'admin/cpfm-feedback/css/cpfm-admin-feedback.css',
            [],
        ...
        wp_enqueue_script(
            ...
            TECC_PLUGIN_URL . 'admin/cpfm-feedback/js/cpfm-admin-feedback.js', 
            ['jquery'], 
            TECC_VERSION_CURRENT, 
            true
    ...
    public function cpfm_render_notice_panel() {
        ...
        $output .= '<div class="notice-panel-header">' . esc_html__('Help Improve Plugins', 'tecc') . ' <span class="dashicons dashicons-no" id="cpfm_remove_notice"></span></div>';
        ...
        foreach (self::$registered_notices as $key => $notice) {
            ...
            $output .= '<p>' . esc_html($notice['message']) . '<a href="#" class="cpf-toggle-extra">' . esc_html__(' More info', 'tecc') . '</a></p>';
            $output .= '</div>';
            
            $output .= '<div class="cpf-extra-info">';
            $output .= '<p>' . esc_html__('Opt in to receive email updates about security improvements, new features, helpful tutorials, and occasional special offers. We\'ll collect:', 'tecc') . '</p>';
            $output .= '<ul>';
            $output .= '<li>' . esc_html__('Your website home URL and WordPress admin email.', 'tecc') . '</li>';
            $output .= '<li>' . esc_html__('To check plugin compatibility, we will collect the following: list of active plugins and themes, server type, MySQL version, WordPress version, memory limit, site language and database prefix.', 'tecc') . '</li>';
            $output .= '</ul>';
            
            $output .= '</div>';
            
            $output .= '<div class="notice-actions">';
            $output .= '<button class="button button-primary opt-in-yes" data-category="' . esc_attr($key) . '" id="yes-share-data" value="yes">' . esc_html__("Yes, I Agree", 'tecc') . '</button>';
            $output .= '<button class="button opt-in-no" data-category="' . esc_attr($key) . '" id="no-share-data" value="no">' . esc_html__('No, Thanks', 'tecc') . '</button>';
            $output .= '</div>';
